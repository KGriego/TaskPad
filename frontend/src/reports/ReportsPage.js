...
class ReportsPage extends Component {
  ...
  loadReports = async () => {
    ...
    httpApi.getWithErrorHandled(`/api/reports`).then(data => {
      this.setState({ reports: data.data, loading: false });
    });
  };

  onAddReport = () => {
    this.setState({ showReportCreate: true });
  };

  onReload = () => this.loadReports();

  onSaveReport = async report => {
    this.setState({ showReportCreate: false, loading: true });
  ...
  onCancelEditReport = () =>
    this.setState({ showReportCreate: false });
